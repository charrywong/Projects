...
//$(function () {
	window.enableExtension = -1;
	chrome.runtime.onMessage.addListener(
		function (request, sender, sendResponse) {
		//单击用户更改插件可用状态，或者页面第一次加载时都会触发该事件
		if (request.enablevalue != undefined) {
			fanyimsg = Number(request.enablevalue);
			window.enableExtension = fanyimsg;
			/* console.log(fanyimsg); //打印当前插件可用状态 */
		}
	});

	//向background页面发送请求，获取当前插件可用状态
	...
	$('body').mouseup(function (e) {
		if(e.target.className.indexOf("googlefanyimark")>-1)
			return;
		...
		if (!pos||window.enableExtension==0)
			return;
		...
		$.get('http://translate.google.cn/translate_a/single?dt=bd&dt=rm&dt=t', isEnglish(pos) == true ? methode_z : methodz_e, function (response) {
			...
			if (creatTransObj(obj).translation != "") {
				...
				$("#googlefanyitip").find("*").addClass("googlefanyimark");
			}
		}, "text");
		...
		function creatTransObj(transArray) {
			...
			if (!$.isArray(transArray)) {
				transResult.warning += "NOT_AN_ARRAY&";
				return transResult;
			};
